...
def convert_file(
    ...
    *,
    ...
    output_path
        Path for the output file. Format inferred from extension:
        .rbxm copies binary as-is, .rbxmx writes XML, .obj exports mesh.
    extract_child_data
        If True, deserialize embedded ChildData and inject top-level MeshData.
    export_obj_mode
        If True, extract and export MeshData as OBJ.
    decompose
        If True (with OBJ export), export per-operation meshes with CFrame transforms.
    """
...
def _export_obj_from_doc(
    ...
    *,
    ...
    export_obj(vertices, indices, output_path, object_name=obj_name)
...
def _inject_mesh_data(doc: RbxDocument, mesh_data: bytes) -> None:
    """Inject MeshData into root PartOperation instances that lack their own."""
    _PART_OP_CLASSES = {'UnionOperation', 'NegateOperation', 'PartOperation'}
    ...
    for inst in needs_injection:
        ...
        log.info('Injected MeshData (%d bytes) into %s', len(mesh_data), inst.class_name)
        injected = True
...
def _try_extract_child_data(doc: RbxDocument) -> RbxDocument | None:
    """If any instance has a ChildData property with embedded RBXM, parse it."""
    for inst in doc.roots:
